...
class TestUIRegistration(unittest.TestCase):
    ...
    def add_registration_clicked_suit(self, id, name, isTakePhoto):
        QTest.mouseClick(self.window.registrationButton,Qt.LeftButton)
        self.window.registrationDialog.nameEdit.setText(name)
        self.window.registrationDialog.idEdit.setText(id)
        if(isTakePhoto):
            QTest.mouseClick(self.window.registrationDialog.photoButton,Qt.LeftButton)
        QTest.mouseClick(self.window.registrationDialog.addButton,Qt.LeftButton)
        self.assertEqual(self.window.registrationDialog.nameEdit.text(), name)
        self.assertEqual(self.window.registrationDialog.idEdit.text(), id)
        self.assertEqual(self.window.registrationDialog.imgEdit.text(), id + '.jpg')
    
    def test_textEditsWidgetsText(self):
        self.assertEqual(self.window.registrationDialog.nameEdit.placeholderText(), '')
        self.assertEqual(self.window.registrationDialog.idEdit.placeholderText(), '')
        self.assertEqual(self.window.registrationDialog.imgEdit.placeholderText(), '')
        self.assertEqual(self.window.registrationDialog.imgEdit.isEnabled(), False)
    
    def test_take_photo_clicked(self):
        self.window.registrationDialog.photoButton.clicked.emit()
        self.assertFalse(self.window.registrationDialog.imgEdit.isEnabled())
